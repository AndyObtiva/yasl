...
RSpec.describe do
  ...
  describe '#dump' do
    ...
    context 'unserializable' do
      before do
        @unserializable_data_types = YASL::UNSERIALIZABLE_DATA_TYPES
      end
      after do
        YASL::UNSERIALIZABLE_DATA_TYPES = @unserializable_data_types
      end
      it 'silently ignores non-serializable objects like `Proc`, `Binding`, and `IO`' do
        ...
        YASL::UNSERIALIZABLE_DATA_TYPES << String
        dump = YASL.dump('Hello, World!')
        expect(dump).to eq("null")
      end
    end
